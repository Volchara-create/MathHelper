...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (token) {
    ...
    authShowUser(user);
    // Handle ?p= redirect from 404.html
    const _redirectPath = new URLSearchParams(location.search).get('p');
    if (_redirectPath) history.replaceState(null, null, _BASE + _redirectPath.replace(/^\//, ''));

    const _rawPath = (_redirectPath || location.pathname).replace(/^\/MathHelper\/?/, '').replace(/^\//, '').split('/')[0].split('?')[0];
    const _initPath = _rawPath;
...
const _BASE = window.location.hostname === 'volchara-create.github.io' ? '/MathHelper/' : '/';

function authOpen(mode) {
  document.getElementById('auth-modal').classList.add('active');
  authSwitch(mode);
  history.replaceState({auth: mode}, '', _BASE + mode);
}
...
function downloadSidePanelPDF() {
  const user = JSON.parse(localStorage.getItem('mh_user') || '{}');
  if (!user.isPro) { showToast('📥 PDF доступний тільки в Pro версії!'); return; }
  ...
  el.innerHTML = `
    <div style="font-family:Georgia,serif;padding:40px;max-width:750px;margin:0 auto;">
      <div style="border-bottom:2px solid #1565c0;padding-bottom:12px;margin-bottom:24px;">
        <h1 style="color:#1565c0;margin:0 0 4px;font-size:24px;">📓 Зошит</h1>
        <p style="color:#888;margin:0;font-size:13px;">MathHelper · ${date}</p>
      </div>
      <div style="font-size:15px;line-height:1.8;color:#222;white-space:pre-wrap;">${content.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
...
function downloadDrawerNotePDF() {
  const user = JSON.parse(localStorage.getItem('mh_user') || '{}');
  if (!user.isPro) {
    showToast('📥 PDF доступний тільки в Pro версії!');
    return;
  }
  const title = document.getElementById('drawer-title-input')?.value || 'Конспект';

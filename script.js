...
const MATHIK_TUTORIAL = [
  // ── 0. Вступ (сова вдома) ──────────────────────────────────
  {
    msg: '👋 Привіт! Я <b>Mathik</b> 🦉 — твій помічник.<br>Зараз зроблю <b>огляд</b> сайту — покажу кожен розділ зсередини. Поїхали! 🚀'
  },
  // ── 1. Формули — летить до вкладки Алгебра ─────────────────
  {
    navigate: () => show('formulas'),
    target: '#ftab-btn-algebra',
    msg: '📐 <b>Розділ Формули!</b><br>Тут вкладки: Алгебра, Геометрія, Тригонометрія. Обери тему → натисни на формулу щоб побачити детальний опис!'
  },
  // ── 2. Відкриває категорію → перша формула ─────────────────
  {
    navigate: () => {
      showFormulaTab('algebra');
      setTimeout(() => {
        const cats = document.querySelectorAll('.alg-cat-btn');
        const q = Array.from(cats).find(b => b.textContent.includes('Квадрат')) || cats[0];
        if (q) q.click();
      }, 200);
    },
    target: '.alg-modal-row',
    msg: '🔢 <b>Список формул категорії!</b><br>Кожен рядок клікабельний — відкриє велику картку з поясненням і прикладом. Спробуй!'
  },
  // ── 3. Квіз ────────────────────────────────────────────────
  {
    navigate: () => { try { closeAlgebraModal(); } catch(e){} show('quiz'); },
    target: '.quiz-topic-btn, .vtask-start-btn, [onclick*="startQuiz"]',
    msg: '🎮 <b>Розділ Квіз!</b><br>Обери тему → відповідай → після кожної відповіді бачиш пояснення. Ідеально перед контрольною!'
  },
  // ── 4. НМТ в нижньому меню (завжди видно) ──────────────────
  {
    target: 'a.qm-btn[href="simulator.html"]',
    msg: '📝 <b>НМТ Симулятор</b> — ось кнопка в нижньому меню!<br>30 питань, 90 хвилин — точно як на реальному НМТ. Після — розбір помилок.'
  },
  // ── 5. Пошук (завжди видно) ────────────────────────────────
  {
    target: '#search-btn',
    msg: '🔍 <b>Пошук</b> — ця кнопка або клавіша <kbd>/</kbd>.<br>Шукає формули, теми, нотатки по всьому сайту.'
  },
  // ── 6. Налаштування (завжди видно) ─────────────────────────
  {
    target: 'button[onclick="openSettings()"]',
    msg: '⚙️ <b>Налаштування</b> — вибери свій клас (7–11) і щоденну ціль.<br>Зареєструйся щоб прогрес зберігався між сесіями!'
  },
  // ── 7. Фінал — повертається додому ─────────────────────────
  {
    isLast: true,
    msg: '🎉 <b>Огляд завершено!</b><br>Тепер ти знаєш усе. Починай з Формул → Квіз.<br>Якщо заблукав — я завжди тут, натисни 🦉! 💪'
  }
];
...
// ===== OWL ANIMATION via transform:translate() =====
// We NEVER change left/top/right/bottom — owl stays at right:16px;bottom:80px always.
// Movement is done via transform:translate(dx, dy). This is bulletproof on position:fixed.

let _owlTX = 0; // current translate X offset from home
let _owlTY = 0; // current translate Y offset from home
...
// Animate translate from (fx,fy) to (tx,ty) over duration ms
function _animOwlTranslate(fx, fy, tx, ty, duration, easing, onDone) {
  ...
  function step(now) {
    ...
    const cx  = fx + (tx - fx) * e;
    const cy  = fy + (ty - fy) * e;
    // Keep scale if owl-flying class is present
    const sc  = bubble.classList.contains('owl-flying') ? ' scale(1.5)' : '';
    bubble.style.transform = `translate(${cx}px,${cy}px)${sc}`;
...
// Fly owl to a CSS selector — stays there after landing
function _owlFlyToAndStay(selector, callback) {
  ...
  _owlFlying = true;
  const bubble  = document.getElementById('mathik-bubble');
  // Home rect WITHOUT current translate (strip translate to get real home position)
  ...
  // Offset from home to destination
  ...
  _animOwlTranslate(_owlTX, _owlTY, toTX, toTY, 1100, _easeOutBack, () => {
    _owlTX = toTX;
    _owlTY = toTY;
...
// Fly owl back to home position (translate → 0,0)
function _owlFlyHome(callback) {
  ...
  _owlFlying = true;
  // Get home position (without translate)
  const hr    = bubble.getBoundingClientRect();
  const homeL = hr.left - _owlTX;
  const homeT = hr.top  - _owlTY;
  // Below-screen Y in translate terms
  const belowTY = (window.innerHeight + 120) - homeT;
  const homeTX  = 0; // home is translate(0,0)

  bubble.classList.add('owl-flying');

  // 1. Shoot down (280ms)
  _animOwlTranslate(_owlTX, _owlTY, homeTX, belowTY, 280, _easeInQuad, () => {
    _owlTX = homeTX;
    _owlTY = belowTY;
    // 2. Spring up to home (500ms)
    _animOwlTranslate(homeTX, belowTY, 0, 0, 520, _easeOutBack, () => {
      _owlTX = 0;
      _owlTY = 0;
      bubble.style.transform = '';
      bubble.classList.remove('owl-flying');
